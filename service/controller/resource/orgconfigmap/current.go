...
func (r *Resource) GetCurrentState(ctx context.Context, obj interface{}) ([]*corev1.ConfigMap, error) {
	...
	{
		r.logger.Debugf(ctx, "finding organization configmaps in namespace %#q", cr.GetNamespace())
		...
		r.logger.Debugf(ctx, "found %d organization configmaps in namespace %#q", len(configMaps), cr.Status.Namespace)
	}
